...
comparisons_directory = 'comparisons3/dt_comparisons/'
data_directory = 'comparisons3/dt_data/'
...
for t in range(TRIALS):
    ...
    if t == 0:
        plotter.plot_state_actions(mdp.pi, rewards=rewards, sinks=sinks,
                filename=comparisons_directory + 'classic_il_state_action.png')        
...
for t in range(TRIALS):
    ...
    for _ in range(ITER):
        ...
        if _ == 0 and t ==0:
            ...
            dagger_analysis.save_states(comparisons_directory + "dagger_final.png")            
            dagger_analysis.show_states()
    if t == 0:
        dagger_analysis.reset_density()        
        dagger_analysis.count_states(dagger.get_states())
        dagger_analysis.save_states(comparisons_directory + "dagger.png")
        dagger_analysis.show_states()
        plotter.plot_state_actions(mdp.pi, rewards=rewards, sinks=sinks,
                filename=comparisons_directory + 'dagger_state_action.png')
    dagger_data[t,:] = r
    dagger_acc[t,:] = acc
    dagger_loss[t,:] = loss
...
print "Saving analysis to: " + comparisons_directory + 'reward_comparison.png'
...
loss_analysis.plot(names = ['DT IL loss', 'DT DAgger loss'], filename=comparisons_directory + 'loss_plot.png', ylims=[0, 1])
