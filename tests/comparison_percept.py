...
plotter.plot_state_actions(value_iter_pi, rewards = grid.reward_states, sinks = grid.sink_states,
        filename='comparisons/percept_comparisons/value_iter_state_action.png')
...
for t in range(TRIALS):
    ...
    percept = Perceptron()
    sup = ScikitSupervise(grid, mdp, Classifier=percept)
    ...
    if t == 0:
        plotter.plot_state_actions(mdp.pi, rewards=rewards, sinks=sinks,
                filename='comparisons/percept_comparisons/percept_classic_il_state_action.png')        
...
np.save('comparisons/percept_data/percept_sup_data.npy', value_iter_data)
np.save('comparisons/percept_data/percept_classic_il_data.npy', classic_il_data)
np.save('comparisons/percept_data/percept_dagger_data.npy', dagger_data)

np.save('comparisons/percept_data/percept_dagger_acc.npy', dagger_acc)
np.save('comparisons/percept_data/percept_classic_il_acc.npy', classic_il_acc)
...
analysis.plot(names = ['Value iteration', 'Percept. IL', 'Percept. DAgger'], filename='comparisons/percept_comparisons/percept_reward_comparison.png', ylims=[-60, 100])
...
acc_analysis.plot(names = ['Percept. IL Acc.', 'Percept. DAgger Acc.'], label='Accuracy', filename='comparisons/percept_comparisons/percept_acc_comparison.png', ylims=[0,1])
