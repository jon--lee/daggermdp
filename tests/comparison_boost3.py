...
plotter.plot_state_actions(value_iter_pi, rewards = grid.reward_states, sinks = grid.sink_states,
        filename='boost3_comparisons/value_iter_state_action.png')
...
for t in range(TRIALS):
    ...
    if t == 0:
        plotter.plot_state_actions(mdp.pi, rewards=rewards, sinks=sinks,
                filename='boost3_comparisons/boost3_classic_il_state_action.png')        
...
for t in range(TRIALS):
    ...
    for _ in range(ITER):
        ...
        if _ == ITER - 1 and t == 0:
            ...
            dagger_analysis.save_states("boost3_comparisons/boost3_dagger_final.png")            
            dagger_analysis.show_states()
    if t == 0:
        dagger_analysis.reset_density()        
        dagger_analysis.count_states(dagger.get_states())
        dagger_analysis.save_states("boost3_comparisons/boost3_dagger.png")
        dagger_analysis.show_states()
        plotter.plot_state_actions(mdp.pi, rewards=rewards, sinks=sinks,
                filename='boost3_comparisons/boost3_dagger_state_action.png')
    dagger_data[t,:] = r
    dagger_acc[t,:] = acc
...
np.save('boost3_data/boost3_sup_data.npy', value_iter_data)
np.save('boost3_data/boost3_classic_il_data.npy', classic_il_data)
np.save('boost3_data/boost3_dagger_data.npy', dagger_data)

np.save('boost3_data/boost3_dagger_acc.npy', dagger_acc)
np.save('boost3_data/boost3_classic_il_acc.npy', classic_il_acc)
...
analysis.plot(names = ['Value iteration', 'AdaBoost IL', 'DT DAgger'], filename='boost3_comparisons/boost3_reward_comparison.png', ylims=[-60, 100])
...
acc_analysis.plot(names = ['AdaBoost IL Acc.', 'DT DAgger Acc.'], label='Accuracy', filename='boost3_comparisons/boost3_acc_comparison.png', ylims=[0,1])
